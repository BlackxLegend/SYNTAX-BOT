...
module.exports = {
	...
	onLoad: async function ({ api }) {
		if (fs.existsSync(dirBootLogTemp)) {
			...
			api.sendMessage("Your bot has been restarted.", threadID);
		}
	},

	onStart: async function ({ message, getLang, commandName, event }) {
		// Check for updates
		const { data: { version } } = await axios.get("https://raw.githubusercontent.com/jarif098/SYNTAX-BOT/main/package.json");
		const { data: versions } = await axios.get("https://raw.githubusercontent.com/jarif098/SYNTAX-BOT/main/versions.json");
